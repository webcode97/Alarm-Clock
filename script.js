...
const ring = new Audio('https://assets.codepen.io/4927073/yt5s.io+-+Post+Malone+-+I+Cannot+Be+%28A+Sadder+Song%29+%28Audio%29+ft.+Gunna+%28128+kbps%29.mp3');
ring.loop = true;
    ...
    var dname = now.getDay(),
        ...
        for(let i=0; i<alarmListArr.length;i++){
            if(alarmListArr[i]==`${hou.pad(2)}:${min.pad(2)}:${sec.pad(2)} ${pe}`){
                ...
                document.querySelector('body').classList.add('bg-animate');
                document.querySelector('.music').classList.add('alarmrun');
...
function setAlarm(){
    ...
    if(time.includes("setHour") || time.includes("setMinute") || time.includes("setSeconds") || time.includes("AM/PM")){
        showalert(`Please, Select Valide Input`);
        ...
        showalert(`Your Alarm Set <mark>${alarmTime}</mark>.`)
    }
...
function deleteAlarm(click_id){
    ...
    showalert(`Your Alarm <mark>${click_id} </mark>Deleted successfully.`);
}

function stopAlarm(){
    ring.pause();
    document.querySelector("#stopAlarm").style.visibility= "hidden";
    document.querySelector('body').classList.remove('bg-animate');
    document.querySelector('.music').classList.remove('alarmrun');
}
